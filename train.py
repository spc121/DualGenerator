    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if epoch == args.nepoch - 1:
                outcom_train.append(out2)
                if (i + 1) % 300 == 0:
                    data_num = (i + 1) // 300
                    path = "data/vrcnet/train/epoch_" + str(epoch)
                    mkdir(path)
                    torch.save(outcom_train, "data/vrcnet/train/epoch_" + str(epoch) + "/outcom_train_" + str(data_num) + ".pt")
                    outcom_train = []
...
def val(net, curr_epoch_num, val_loss_meters, dataloader_test, best_epoch_losses):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test):
            ...
            if curr_epoch_num == 49:
                outcom_test.append(result_dict['out2'])
                if (i + 1) % 200 == 0:
                    data_num = (i + 1) // 200
                    path = "data/vrcnet/test/epoch_" + str(curr_epoch_num)
                    mkdir(path)
                    torch.save(outcom_test, "data/vrcnet/test/epoch_" + str(curr_epoch_num) + "/outcom_test_" + str(data_num) + ".pt")
                    outcom_test = []
        ...
        # torch.save(outcom_test, "data/vrcnet/outcom_test_" + str(curr_epoch_num) + ".pt")
