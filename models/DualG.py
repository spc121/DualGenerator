...
from models.seedformer import SeedGenerator
from models.seedformer import FeatureExtractor
from models.utils import fps_subsample
from models.loss_utils import get_loss
from models.stylegan import StyleGenerator, StyleDiscriminator, R1Penalty, R2Penalty
...
class CosineSimilarity(nn.Module):
    ...
    def forward(self, tensor_1, tensor_2):
        ...
        normalized_tensor_1 = normalized_tensor_1.view(-1)
        normalized_tensor_2 = normalized_tensor_2.view(-1)
        # normalized_tensor_1 = tensor_1.view(-1)
        # normalized_tensor_2 = tensor_2.view(-1)
        c = cosine(normalized_tensor_1.detach().cpu().numpy(), normalized_tensor_2.detach().cpu().numpy())
        return c

class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        super(Model, self).__init__()
        self.w1 = nn.Parameter(torch.tensor([0.75]))
        ...
        DATA_DIR = os.path.join(BASE_DIR, 'completion', 'models', 'stylegan', 'models')
        mkdir(DATA_DIR)
        self.s_left = 0.90
        self.s_right = 1.10
        self.tip = 0
        ...
        self.encoder_com = PCN_encoder(output_size=global_feature_size)
        self.encoder = FeatureExtractor(out_dim=global_feature_size, n_knn=args.knn_list)
        ...
        self.generator_com = Linear_ResBlock(input_size=size_z, output_size=global_feature_size)
        self.generator = SeedGenerator(feat_dim=global_feature_size, seed_dim=128, n_knn=16, factor=2, attn_channel=True)
        ...
        self.cos = CosineSimilarity(tensor_1, tensor_2)
        self.G = StyleGenerator()
        self.D = StyleDiscriminator()
        self.optim_D = optim.Adam(self.D.parameters(), lr=0.00001, betas=(0.5, 0.999))
        self.optim_G = optim.Adam(self.G.parameters(), lr=0.00001, betas=(0.5, 0.999))
        self.scheduler_D = optim.lr_scheduler.ExponentialLR(self.optim_D, gamma=0.9995)
        self.scheduler_G = optim.lr_scheduler.ExponentialLR(self.optim_G, gamma=0.9995)
        self.softplus = nn.Softplus()
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.r1_gamma = 10.0
        self.r2_gamma = 0.0
        self.path_G = DATA_DIR + "/latestG.pth"
        self.path_D = DATA_DIR + "/latestD.pth"
    ...
    def cyclegan_val(self, course):
        course = course[:1, :512]
        # real = Variable(course_s).cuda()  ## 真图像A
        # model_fineA = self.G(torch.randn([course.size(0), 512]).to(self.device))
        model_fineA = self.G(course.to(self.device))
        fine_feat = torch.squeeze(model_fineA)
        fine_feat = fine_feat[:, :128, :128]
        fine_feat = fine_feat.reshape(8, 2048, 3)
        model_fineA = fine_feat[:4, :, :]
        return model_fineA

    def stylegan(self, gt):
        self.G.to(self.device)
        self.D.to(self.device)
        gt = gt.view(1, 3, 128, 128)
        gt = gt.repeat((1, 1, 8, 8))

        self.D.zero_grad()
        real_img = gt
        real_logit = self.D(real_img).to(self.device)
        fake_img = self.G(torch.randn([real_img.size(0), 512]).to(self.device))
        fake_logit = self.D(fake_img.detach())

        d_loss = self.softplus(fake_logit).mean()
        d_loss = d_loss + self.softplus(-real_logit).mean()

        if self.r1_gamma != 0.0:
            r1_penalty = R1Penalty(real_img.detach(), self.D)
            d_loss = d_loss + r1_penalty * (self.r1_gamma * 0.5)
        if self.r2_gamma != 0.0:
            r2_penalty = R2Penalty(fake_img.detach(), self.D)
            d_loss = d_loss + r2_penalty * (self.r2_gamma * 0.5)
        ...
        g_loss = self.softplus(-fake_logit).mean()
        ...
        real_img = torch.squeeze(real_img)
        real_img = real_img[:, :128, :128]
        fake_img = torch.squeeze(fake_img)
        fake_img = fake_img[:, :128, :128]

        return real_img, fake_img, g_loss, d_loss
    ...
    def forward(self, x, gt=None, prefix="train", mean_feature=None, alpha=None):
        ...
        if prefix=="train":
            ...
            points = x
        feat, patch_xyz, patch_feat = self.encoder(points)
        # print("333", patch_feat.size())
        ...
        if prefix=="train":
            ...
            z_feat = self.generator_com(z)
            t_feat = z_feat.view(batch_size_train, -1, 1)
            ...
            z_feat = self.generator_com(z)
            t_feat = z_feat.view(batch_size_test, -1, 1)

        seed, seed_feat = self.generator(t_feat, patch_xyz, patch_feat)
        seed = seed.permute(0, 2, 1).contiguous()  # [B, 256, 3]
        points = points.permute(0, 2, 1).contiguous()
        pcd = fps_subsample(torch.cat([seed, points], 1), 2048)  # (B/32, 3, 2048)
        pcd = pcd.permute(0, 2, 1).contiguous()
        sefeat = self.encoder_com(pcd)
        feat += sefeat   # feat += z_feat
        pcd = pcd.permute(0, 2, 1).contiguous()
        # if prefix == "val":
        #     pcd = torch.cat([pcd, pcd], dim=0)
        pcd_clone = pcd.clone().detach()

        if prefix == "train":
            real_A, fake_A, loss_G, loss_D = self.stylegan(pcd_clone)
            fine_feat = self.w1*real_A + (1-self.w1)*fake_A
            fine_feat = fine_feat.view(8, 2048, 3)
            # print('G', loss_G)
            # print('D', loss_D)
        elif prefix == "val":
            # pcd = torch.cat([pcd, pcd], dim=0)
            fine_feat = self.cyclegan_val(sefeat)
        ...
        for i in range(fine.shape[0]):
            score_fine = self.cos(fine_feat[i], fine[i])  # gt[i]   3485
            # print("$$$$$$$$$", score_fine)
            if score_fine < 0.98 or score_fine > 1.02:
                fine[i] = self.w2 * fine[i].clone() + (1 - self.w2) * fine_feat[i].clone().detach()
        # print("w3", self.w3)
        # for i in range(fine.shape[0]):
        #     score_fine = self.cos(fine_feat[i], fine[i])  # gt[i]   3485
        #     # print("$$$$$$$$$", score_fine)
        #     if score_fine < max(0.95, self.s_left) or score_fine > min(1.05, self.s_right):
        #         weit = abs(math.sin(score_fine * math.pi / 2))* self.w2
        #         # print("1111", weit)
        #         fine[i] = weit * fine[i].clone() + (1 - weit) * fine_feat[i].clone().detach()
        #         self.tip = self.tip + 1
        #     # print("w3", self.w3)
        # if self.tip <= fine.shape[0]//4:
        #     self.s_left = self.s_left + 0.005
        #     self.s_right = self.s_right - 0.005
        #     self.s_left = min(self.s_left, 0.99)
        #     self.s_right = max(self.s_right, 1.01)
        # self.tip = 0
        ...
        if prefix=="train":
            ...
            elif self.distribution_loss == 'EMD':
                z_m = m_distribution.rsample()
                z_m = z_m.view(-1)
                z_m_np = z_m.cpu().detach().numpy()
                z_q = q_distribution.rsample()
                z_q = z_q.view(-1)
                z_q_np = z_q.cpu().detach().numpy()
                z_p = p_distribution.rsample()
                z_p = z_p.view(-1)
                z_p_np = z_p.cpu().detach().numpy()
                z_p_fix = p_distribution_fix.rsample()
                z_p_fix = z_p_fix.view(-1)
                z_p_fix_np = z_p_fix.cpu().detach().numpy()
                dl_rec = scipy.stats.wasserstein_distance(z_m_np, z_p_np)
                dl_g = scipy.stats.wasserstein_distance(z_q_np, z_p_fix_np)
            else:
            ...
            PMloss = get_loss(fine, gt, sqrt=True)

            total_train_loss = loss1.mean() * 10 + loss2.mean() * 0.5 + loss3.mean() + loss4.mean() * alpha + PMloss.mean() * alpha * 0.5
